...
export default function ReceivedResponseDetailPage() {
  ...
  const letter = {
    ...
    answer: `안녕하세요! 답장입니다. 이 편지는 테스트용으로 작성되었습니다. 내용은 자유롭게 작성해 주세요. 편지의 내용은 나중에 수정할 수 없습니다. 편지를 작성하는 것은 정말 재미있고 창의적인 작업입니다. 편지를 통해 감정을 표현하고, 생각을 나누는 것은 소중한 경험이 될 것입니다. 편지를 쓰는 동안 즐거운 시간을 보내세요!`,
  };
  return (
    <>
      <Appbar title={`편지 ${responseId}`} />
      <div>
        <p>{letter.date}</p>
        <h2>{letter.title}</h2>
        <p>{letter.content}</p>
